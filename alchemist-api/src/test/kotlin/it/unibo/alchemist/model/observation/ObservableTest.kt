...
class ObservableTest : FunSpec({
    ...
    context("`Observable` extensions tests") {
        ...
        context("`Observable<Option<T>>` extensions tests") {
        ...
        context("flatMap") {
            ...
            test("should unsubscribe from inner observables on stopWatching") {
                ...
                fused.onChange(this) { }
                set.add("b")
                inner.getValue("a").observers.shouldHaveSize(1)
                inner.getValue("b").observers.shouldHaveSize(1)
                fused.stopWatching(this)
                inner.getValue("a").observers.shouldHaveSize(0)
                inner.getValue("b").observers.shouldHaveSize(0)
            }
        }
