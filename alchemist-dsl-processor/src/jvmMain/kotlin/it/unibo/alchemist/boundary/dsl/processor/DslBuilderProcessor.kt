...
class DslBuilderProcessor(private val codeGenerator: CodeGenerator, private val logger: KSPLogger) : SymbolProcessor {
    ...
    fun buildGenerationContext(
        ...
        // Track which parameters are injected so we can include them in the signature/context.
    ...
    private fun processClass(classDeclaration: KSClassDeclaration) {
        ...
        contextualizableConstructors.forEach { constructor ->
            val typeParameters = constructor.typeParameters
            ...
            writer.println(
                """
                    inline fun $typeParameters $functionName() = TODO()
                """.trimIndent(),
            )
        }
...
//        writeGeneratedFile(classDeclaration, generationContext)
    ...
    fun writeGeneratedFile(classDecl: KSClassDeclaration, context: GenerationContext) {
    ...
    private fun writeFileHeader(writer: PrintWriter, classDecl: KSClassDeclaration, contextType: ContextType) {
        ...
        when (contextType) {
            ContextType.SIMULATION_CONTEXT -> writer.println(
                "import ${ProcessorConfig.ContextTypes.SIMULATION_CONTEXT}",
            )
            ...
            ContextType.DEPLOYMENTS_CONTEXT -> writer.println(
                "import ${ProcessorConfig.ContextTypes.DEPLOYMENTS_CONTEXT}",
            )
