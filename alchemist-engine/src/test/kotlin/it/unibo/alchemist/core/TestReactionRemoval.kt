...
class TestNodeReaction<T>(
    ...
    override fun cloneOnNewNode(node: Node<T>, currentTime: Time): NodeReaction<T> = notImplementedError()
}
...
class TestReactionRemoval : FreeSpec({
    "Reactions can be removed from simulation" {
        ...
        val customReactions = listOf(TestNodeReaction(node, DiracComb(1.0)))
