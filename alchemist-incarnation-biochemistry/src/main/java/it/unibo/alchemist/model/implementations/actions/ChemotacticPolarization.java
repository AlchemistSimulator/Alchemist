...
public final class ChemotacticPolarization<P extends Position2D<P>> extends AbstractAction<Double> {
    ...
    public ChemotacticPolarization<P> cloneAction(final Node<Double> node, final Reaction<Double> reaction) {
        return new ChemotacticPolarization<>(environment, node, biomol.toString(), ascend ? "up" : "down");
    }
    ...
    public void execute() {
        ...
        if (l.isEmpty()) {
            thisNode.asProperty(CellularProperty.class).addPolarizationVersor(environment.makePosition(0, 0));
        } else {
            ...
            if (isNodeOnMaxConc) {
                thisNode.asProperty(CellularProperty.class)
                        .addPolarizationVersor(environment.makePosition(0, 0));
            } else {
                ...
                if (newPolVerModule == 0) {
                    ...
                    if (ascend) {
                        ...
                        thisNode.asProperty(CellularProperty.class)
                                .addPolarizationVersor(environment.makePosition(-newPolVer.getX(), -newPolVer.getY()));
                    }
                }
            }
        }
