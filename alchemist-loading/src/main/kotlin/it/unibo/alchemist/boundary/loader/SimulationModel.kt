...
private fun buildJSR223Variable(
    name: String,
    language: String,
    formula: String,
    timeout: Any?,
 ...
 * UTILITY EXTENSIONS
 */
private inline fun <T> T.whenNull(
    and: Boolean = true,
    then: () -> T,
): T = if (this == null && and) then() else this
...
private fun Map<*, *>.takeIfNotAConstant(
    name: String,
    context: Context,
) = takeUnless { name in context.constants }
...
private fun String.toTimeout(name: String): Long =
    runCatching { toLong() }.getOrElse {
...
internal object SimulationModel {
    ...
    internal fun <P : Position<P>, T : Any?> visitLinkingRule(
    ...
    internal fun inject(
        context: Context,
        root: Map<String, *>,
    ): Map<String, Any> = (replaceKnownRecursively(context, root) as Map<String, Any>).also { logger.debug("New model: {}", it) }

    private fun makeDefaultRandomGenerator(seed: Long) = MersenneTwister(seed)

    private fun replaceKnownRecursively(
        context: Context,
        root: Any?,
    ): Any? =
        when (root) {
    ...
    internal inline fun <reified T : Any> visitBuilding(
        context: Context,
        root: Map<*, *>,
    ): Result<T>? =
    ...
    private fun visitDependentVariable(
        name: String,
        context: Context,
        root: Any?,
        ...
        return when {
            ...
            root.containsKey(formulaKey) ->
                when (val formula = root[formulaKey]) {
    ...
    private fun visitDependentVariableRegistering(
        name: String,
        context: Context,
        root: Any?,
    ...
    private fun visitJVMConstructor(
        context: Context,
        root: Map<*, *>,
    ): JVMConstructor? =
    ...
    fun <T, P : Position<P>> visitLayers(
        incarnation: Incarnation<T, P>,
        context: Context,
        root: Any?,
    ...
    private fun visitRandomGenerator(
        context: Context,
        root: Any,
    ): RandomGenerator =
        when (root) {
    ...
    fun <T, P : Position<P>> visitProgram(
        ...
        if (ProgramSyntax.validateDescriptor(program) || GlobalProgramSyntax.validateDescriptor(program)) {
            ...
            fun <R : Any> visitIncarnationBuildable(
                ...
                fun <R> create(
                    parameter: Any?,
                    makeWith: ReactionComponentFunction<T, P, R>,
                ): Result<R> =
                    runCatching {
    ...
    fun visitSeeds(
        ...
        when (root) {
            null ->
                makeDefaultRandomGenerator(0) to
    ...
    private fun <T : Any> visitRecursively(
         ...
         */
        fun tryVisit(): Result<T>? =
            visitSingle(root)?.onSuccess {
        ...
        return when (root) {
            ...
            is Map<*, *> -> {
                ...
                fun recurse(previousResult: Result<T>?): List<T> =
                    visitRecursively(
                        evidence,
                        context,
                        root.values,
