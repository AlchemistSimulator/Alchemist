...
import it.unibo.alchemist.model.randomAngle
...
class ZigZagRandomTarget<T>(
    ...
    private fun changeDirection() = with(randomGenerator.randomAngle()) {
        if (minChangeInDirection > 0.0 && abs(this - direction) < minChangeInDirection) {
            ...
            this + minChangeInDirection * if (randomGenerator.nextBoolean()) 1.0 else -1.0
        } else {
            this
        }.also {
            direction = it
