...
/**
 * Moves the pedestrian where the scalar field is lower.
 */
class AvoidScalarField<T, P, A>(
    env: Environment<T, P>,
    reaction: Reaction<T>,
    ...
    override fun Sequence<P>.selectPosition(currentPosition: P): P = this
        .minBy { concentrationIn(it) }
        ?.takeIf { concentrationIn(it) < concentrationIn(currentPosition) }
        ?: currentPosition

    override fun cloneAction(n: Node<T>, r: Reaction<T>): Action<T> =
        requireNodeTypeAndProduce<Pedestrian<T, P, A>, AvoidScalarField<T, P, A>>(n) {
            AvoidScalarField(env, reaction,  it, concentrationIn, center)
        }
}
