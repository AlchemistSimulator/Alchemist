...
public class TestJunction {
    ...
    public void test() {
        ...
        j1.setNeighborNode(node2);
        node1.addJunction(j1);
        ...
        assertEquals(node1.getJunctions().size(), 1);
        assertEquals(node2.getJunctions().size(), 0);
        assertEquals(node3.getJunctions().size(), 0);

        final Junction j2 = new Junction(jBase);
        j2.setNeighborNode(node3); // same junction type of j1, but different neighbor.
        node1.addJunction(j2);
        assertTrue(node1.containsJunction(j1));
        ...
        assertEquals(node1.getJunctions().size(), 2);
        assertEquals(node2.getJunctions().size(), 0);
        assertEquals(node3.getJunctions().size(), 0);
        ...
        for (int i = 0; i < totJ; i++) { // add many identical junction to node 2
            ...
            jtmp.setNeighborNode(node3);
            node2.addJunction(jtmp);
        }
        /* Situation Summary: 
         ...
         */
        assertEquals(node1.getJunctions().size(), 2);
        assertEquals(node2.getJunctions().size(), totJ);
        assertEquals(node3.getJunctions().size(), 0);
        /* **** Remove junctions **** */
        ...
        assertEquals(node1.getJunctions().size(), 0);
        assertEquals(node2.getJunctions().size(), totJ);
        assertEquals(node3.getJunctions().size(), 0);
