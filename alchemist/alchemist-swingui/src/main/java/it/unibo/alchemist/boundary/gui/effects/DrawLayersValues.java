...
public abstract class DrawLayersValues extends AbstractDrawLayers implements FunctionDrawer {
     ...
     */
    @Override
    public <T, P extends Position2D<P>> void drawLayers(final Collection<Layer<T, P>> toDraw, final Environment<T, P> env, final Graphics2D g, final BidimensionalWormhole<P> wormhole) {
        if (mapper == null) {
            mapper = createMapper();
        }
     ...
     */
    @Override
    public abstract <T, P extends Position2D<P>> void drawFunction(Function<? super P, ? extends Number> f, Environment<T, P> env, Graphics2D g, BidimensionalWormhole<P> wormhole);
